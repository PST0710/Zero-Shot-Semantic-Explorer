...
void PX4CtrlFSM::process()
{
	...
	{
		debug_msg = controller.calculateControl(des, odom_data, imu_data, u); // 这里计算到的姿态角速度和油门的值给到u, 然后通过u给到mavros并发布出去，姿态模式只需要姿态和油门量
...
void PX4CtrlFSM::goalCallback(const geometry_msgs::PoseStamped::ConstPtr& msg){
	if(state == AUTO_HOVER){
		ROS_INFO("goal receive");

		publish_trigger(odom_data.msg);
	}else{
		ROS_WARN("PX4 Ctrl NOT in hover mode , goal reject");
	}
}
