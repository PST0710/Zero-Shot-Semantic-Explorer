...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber goal_sub = 
        nh.subscribe("/move_base_simple/goal",
                     1,
                     &PX4CtrlFSM::goalCallback,
                     &fsm);
    ...
    ros::Subscriber imu_sub =
        nh.subscribe<sensor_msgs::Imu>("/uav1/mavros/imu/data", // Note: do NOT change it to /mavros/imu/data_raw !!!
                                       100,
    ...
    fsm.ctrl_FCU_pub = nh.advertise<mavros_msgs::AttitudeTarget>("/uav1/mavros/setpoint_raw/attitude", 10);
    fsm.traj_start_trigger_pub = nh.advertise<geometry_msgs::PoseStamped>("/traj_start_trigger", 10);

    fsm.debug_pub = nh.advertise<quadrotor_msgs::Px4ctrlDebug>("/debugPx4ctrl", 10); // debug

    fsm.set_FCU_mode_srv = nh.serviceClient<mavros_msgs::SetMode>("/uav1/mavros/set_mode");
    fsm.arming_client_srv = nh.serviceClient<mavros_msgs::CommandBool>("/uav1/mavros/cmd/arming");
    fsm.reboot_FCU_srv = nh.serviceClient<mavros_msgs::CommandLong>("/uav1/mavros/cmd/command");
